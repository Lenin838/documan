# Phase 29 Implementation Plan v2 — System-Wide Release Certificate Compliance Drift & Post-Certification Variance Audit Engine

> **Authoritative Technical Implementation Specification**
>
> **Author**: Documan Core Engineering & Product Architecture Team
> **Status**: APPROVED IMPLEMENTATION PLAN (v2 REVISION)
...
Phase 29 answers:
- **WHAT** changed in live state since certification?
- **FROM** what certified state ($T_{\text{cert}}$)?
- **TO** what current live state ($T_{\text{now}}$)?
- **WHY** is it classified as variance?
- **WHAT** evidence supports the classification?

It provides release managers, governance officers, and technical stewards with a structured **5-dimensional post-certification variance taxonomy** (Topology, Baseline, Contract, Waiver, and Attestation Variance), evaluates a **4-tier Live Compliance Status** (`FULLY_COMPLIANT`, `COMPLIANT_WITH_EXCEPTIONS`, `NON_COMPLIANT_DRIFT`, `INDETERMINATE_EVIDENCE`), and outputs deterministic, evidence-backed variance explanations with **zero database writes, zero audit log writes, zero background queue workers, zero automated task creation, and zero live state mutations.**
...
1. **LIVE STATE IS COMPARISON TARGET ONLY**:
   Live state ($T_{\text{now}}$) is strictly a comparison target. It MUST NEVER:
   - Overwrite or mutate historical certificate evidence;
   - Reinterpret the historical certificate snapshot;
   - Mutate the `SystemReleaseCertificate` model or lifecycle state;
   - Replace missing historical snapshot fields with current state;
   - Silently convert historical certified state into current state.

2. **VARIANCE != NON-COMPLIANCE**:
   Variance represents a measured delta between $T_{\text{cert}}$ and $T_{\text{now}}$. Non-breaking baseline advances (`VERSION_ADVANCED`), topology additions (`NODE_ADDED`), carried-forward waivers (`CARRIED_FORWARD`), or attestation evidence additions (`EVIDENCE_ADDED`) are measured variances, but do **NOT** automatically constitute `NON_COMPLIANT_DRIFT`.

3. **CERTIFICATE LIFECYCLE STATUS != CURRENT DRIFT STATUS**:
   Certificate lifecycle status (`ACTIVE`, `REVOKED`, `SUPERSEDED`) represents the formal status of the certificate itself. Current drift status represents live system compliance relative to the certificate snapshot. Auditing drift **NEVER** mutates or redefines certificate lifecycle status:
   - `REVOKED` + `NON_COMPLIANT_DRIFT` remain two independent facts.
   - `SUPERSEDED` + `FULLY_COMPLIANT` remain two independent facts.

---

## 1. Repository-Grounded Investigation

The implementation plan is grounded directly in existing repository models and services:
...
1. **Phase 27 Certificate Snapshot (`SystemReleaseCertificate`)**:
   ...
   - Snapshot properties: `snapshot.topologyNodes`, `snapshot.topologyEdges`, `snapshot.activeBaselines`, `snapshot.activeAttestations`, `snapshot.activeWaivers`, `snapshot.evidenceSummary`.
2. **Phase 27 `/verify` Behavior (`system-release-certificate.service.ts`)**:
   - `verifyCertificateIntegrity` verifies SHA-256 certificate hash (`INTEGRITY_VERIFIED` vs `TAMPER_DETECTED`), lifecycle status (`ACTIVE`/`REVOKED`/`SUPERSEDED`), and high-level boolean match (`matchesCertifiedState: boolean`).
   - Phase 29 consumes `/verify` integrity results and extends them with granular multi-dimensional variance auditing.
3. **Phase 28 Release Lineage Engine (`system-release-lineage.service.ts`)**:
   - Handles snapshot-to-snapshot ($T_{\text{cert1}}$ vs $T_{\text{cert2}}$) comparison. Phase 29 reuses Phase 28 helper diffing algorithms, adapting them for $T_{\text{cert}}$ vs $T_{\text{now}}$ comparison.
...
7. **Phase 26 Contract Change Planning Authority (`system-contract-plan.service.ts`)**:
   - Remains the sole authority for candidate change action synthesis and draft change package generation. Phase 29 does NOT duplicate change planning or package synthesis.
...
- Expose deterministic, evidence-backed variance explanations and informational next-review considerations.
- Remain 100% read-only, request-scoped, and deterministic.

### Explicit Prohibitions & Non-Goals (No Remediation Engine)
- **NO Remediation Engine / Task Creation**: Does NOT automatically execute repairs, create `DocumentationWorkRequest` records, or generate Jira/GitHub tickets. (Phase 26/15/16 remain sole authorities for change planning and packages).
...
Phase 29 defines exact, deterministic comparison semantics for 5 post-certification variance dimensions:
...
> **Rule**: Baseline advancement (`VERSION_ADVANCED`) is a measured structural variance, but is **NOT** automatically non-compliant unless it introduces breaking contract deltas or un-waived gate failures.
...
## 9. Compliance Classification Matrix

Phase 29 maps measured variances across all 5 dimensions to a 4-tier **Live Compliance Status**:

```text
+--------------------------+-----------------------------------------------------------------------------------+
| Compliance Status        | Deterministic Mapping Rules                                                       |
+--------------------------+-----------------------------------------------------------------------------------+
| FULLY_COMPLIANT          | - 0 material variances across all 5 dimensions.                                   |
|                          | - Live gate status === PASSED.                                                    |
|                          | - 0 contract deltas, 0 baseline changes, 0 waiver expirations.                    |
+--------------------------+-----------------------------------------------------------------------------------+
| COMPLIANT_WITH_EXCEPTIONS| - Live state contains non-breaking baseline advances (VERSION_ADVANCED).          |
|                          | - Live state contains non-breaking contract additions (ENDPOINT_ADDED).           |
|                          | - Active carried forward waivers (CARRIED_FORWARD) or newly granted waivers exist. |
|                          | - Live gate status === PASSED or PASSED_WITH_WAIVER.                              |
+--------------------------+-----------------------------------------------------------------------------------+
| NON_COMPLIANT_DRIFT      | - Certified waiver has expired (EXPIRED_POST_CERTIFICATION, T_now > expiresAt).  |
|                          | - Certified waiver explicitly revoked (REVOKED_POST_CERTIFICATION).               |
|                          | - Live state contains BREAKING contract deltas (ENDPOINT_REMOVED, FIELD_REMOVED). |
|                          | - Certified baseline deactivated or removed without replacement.                 |
|                          | - Live gate status === BLOCKED.                                                   |
+--------------------------+-----------------------------------------------------------------------------------+
| INDETERMINATE_EVIDENCE   | - Total applicable contracts N_applicable === 0 (ZERO_APPLICABLE_EVIDENCE).       |
|                          | - Connected project unreadable due to ACL boundary or missing evidence.           |
|                          | - Connected projects count > MAX_CONNECTED_PROJECTS (50).                         |
...
## 10. Certificate Lifecycle Semantics vs Drift Status

Phase 29 explicitly decouples **Certificate Lifecycle Status** from **Current Compliance Drift Status**:

$$\mathbf{\text{Certificate Lifecycle Status} \neq \text{Current Compliance Drift Status}}$$

1. **Comparison Baseline Eligibility**:
   Historical certificates with lifecycle states `ACTIVE`, `SUPERSEDED`, or `REVOKED` **MAY ALL BE SELECTED** as historical comparison baselines for compliance drift analysis.
   - *Rationale*: The `SystemReleaseCertificate.snapshot` is immutable historical truth ($T_{\text{cert}}$). Technical stewards frequently need to audit live drift against a superseded release (`v1.0`) or a revoked release (`v2.0-rc1`) to understand operational divergence.
2. **Zero Mutation Rule**:
   Evaluating compliance drift **NEVER** mutates `certificateStatus`, `certificateHash`, or `lifecycleEvents`.
3. **Independent Fact Pairings**:
   - `SUPERSEDED` + `FULLY_COMPLIANT`: The certificate was superseded by a newer release, but current live state remains structurally compatible with the historical snapshot.
   - `REVOKED` + `NON_COMPLIANT_DRIFT`: The certificate was revoked, and live state has also drifted out of compliance.
...
PHASE 29 /compliance-drift-audit
...
- Deterministic, Evidence-Backed Variance Explanations & Next-Review Considerations
```
...
## 15. API Design & Repository Justification
...
### Justification of `POST` vs `GET`
1. **Repository Consistency**: Phase 28 established `POST /api/v1/release-certificates/compare` for request-scoped comparison queries. Phase 29 aligns with this pattern.
2. **Complex Query Payload**: Using `POST` allows structured JSON input payloads without URL length constraints or parameter encoding issues.
3. **Idempotence & Safety**: The endpoint is 100% read-only and idempotent. `POST` is used purely as an analytical query vehicle.
...
export interface ReleaseCertificateComplianceAuditDTO {
  ...
  varianceExplanations: string[]; // Deterministic, evidence-backed text explanations
  nextReviewConsiderations: string[]; // Informational, non-executable review guidance
  varianceSummary: {
...
4. **Variance Explanation & Informational Considerations Panel**:
   - Bulleted, evidence-backed explanations for variance and non-executable review considerations.
...
- **Scenarios 6–10**: Topology Variance detection (`NODE_ADDED`, `EDGE_REMOVED`).
- **Scenarios 11–15**: Baseline Variance vs Compliance (`VERSION_ADVANCED` returns `COMPLIANT_WITH_EXCEPTIONS`).
- **Scenarios 16–20**: Contract Variance vs Compliance (`ENDPOINT_REMOVED` returns `NON_COMPLIANT_DRIFT`, Phase 23 integration).
- **Scenarios 21–25**: Waiver Variance vs Compliance (`EXPIRED_POST_CERTIFICATION` returns `NON_COMPLIANT_DRIFT`).
- **Scenarios 26–30**: Attestation Variance detection (`EVIDENCE_UNCHANGED`, `EVIDENCE_STALE`).
- **Scenarios 31–35**: Live Compliance Status Precedence (`INDETERMINATE` > `NON_COMPLIANT` > `EXCEPTIONS` > `FULLY_COMPLIANT`).
- **Scenarios 36–40**: Certificate Lifecycle vs Drift Independence (`SUPERSEDED` + `FULLY_COMPLIANT`, `REVOKED` + `NON_COMPLIANT_DRIFT`).
...
3. **Variance != Non-Compliance**: Measured variance does not automatically equal non-compliance.
4. **Lifecycle Status != Drift Status**: Certificate lifecycle status is independent of current compliance drift.
5. **Zero Certificate Mutations**: Phase 29 query execution NEVER mutates certificates or hashes.
6. **Zero Automatic Revocations / Re-Certifications**: No automated lifecycle status transitions.
7. **No Live-State Substitution**: Historical snapshot evidence is never replaced by live state.
8. **Strict ACL Graph Isolation**: Unauthorized connected projects are 100% pruned.
9. **Zero DB Writes**: Read-only query execution (persistence = 0).
10. **Pure Deterministic Precedence**: Compliance status evaluated deterministically.
11. **Zero $N+1$ Queries**: Single bulk `$in` queries used for multi-project fetches.
12. **Zero Remediation Engine / Task Automation**: No task creation, ticket creation, or automated repair.
