...
- **Build Command:**
  ...
  ```
- **Pre-Deploy Command:** `LEAVE EMPTY / NOT USED` *(Render Free Web Services do not support Pre-Deploy Commands)*
- **Start Command:**
  ```bash
  node apps/api/dist/scripts/sync-indexes.js && node apps/api/dist/server.js
  ```
*Note on Render Free Startup:* Render Free Web Services do not support Pre-Deploy Commands (a feature restricted to paid compute tiers). Index synchronization is executed during container startup by chaining `node apps/api/dist/scripts/sync-indexes.js && node apps/api/dist/server.js`. The compiled `sync-indexes.js` script is already generated by the existing API build (`tsc -p tsconfig.json`), statically registers all 28 Mongoose models, and runs `syncIndexes()` idempotently before `server.js` boots. The `&&` operator guarantees that if index synchronization fails, the Express server will not launch. Zero application source code modifications are required.
...
- **Repository Command:** Build: `pnpm install --frozen-lockfile && pnpm --filter @documan/api build`, Pre-deploy: `LEAVE EMPTY / NOT USED`, Start: `node apps/api/dist/scripts/sync-indexes.js && node apps/api/dist/server.js`.
- **Expected Result:** Render deploys API service to `https://<app>.onrender.com`.

### PHASE E — Database Index Synchronization Verification
- **Platform:** Render Free Service Startup Log
- **Repository Command:** `node apps/api/dist/scripts/sync-indexes.js` (Executed automatically during Start Command)
- **Expected Result:** Log confirms 28 models registered and synchronized cleanly prior to Express HTTP server startup.
